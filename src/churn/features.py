...
CONTRACT_TO_MONTHS = {"Monthly": 1, "Annual": 12}
...
def _require_columns(df: pd.DataFrame, dataset: str, required: set[str]) -> None:
...
def usage_aggregates(
    ...
    out = agg_30.merge(agg_90, on="customer_id", how="outer").merge(
        prior, on="customer_id", how="outer"
    )
    ...
    fill = {
        ...
        "recent_30_avg_sessions": 0.0,
        "prior_30_avg_sessions": 0.0,
    }
...
def next_renewal_date(
    start: pd.Series, snapshot: pd.Timestamp, cycle_months: pd.Series
) -> pd.Series:
    """Next contract anniversary strictly after the snapshot."""
    months_elapsed = (snapshot.year - start.dt.year) * 12 + (snapshot.month - start.dt.month)
...
def payment_aggregates(
    ...
    failed_recent = (
        recent.assign(is_failed=lambda d: (d["payment_status"] == "failed").astype(int))
        .groupby("customer_id", as_index=False)
...
def build_customer_features(
    ...
    _require_columns(customers, "customers", CUSTOMER_FEATURE_COLUMNS)
    _require_columns(subscriptions, "subscriptions", SUBSCRIPTION_FEATURE_COLUMNS)
    _require_columns(usage, "usage", USAGE_FEATURE_COLUMNS)
    _require_columns(payments, "payments", PAYMENT_FEATURE_COLUMNS)
    ...
    contract_months = base["contract_type"].map(CONTRACT_TO_MONTHS).fillna(1).astype(int)
    base["next_renewal_date"] = next_renewal_date(
        base["subscription_start_date"], snapshot, contract_months
    )
    days_to_renewal = (base["next_renewal_date"] - snapshot).dt.days
    base["renewal_near_flag"] = ((base["churn_flag"] == 0) & days_to_renewal.between(0, 45)).astype(
        int
    )

    features = base.merge(
        usage_aggregates(usage, observation_dates), on="customer_id", how="left"
    ).merge(
        payment_aggregates(payments, subscriptions, observation_dates), on="customer_id", how="left"
    )
    ...
    features["avg_monthly_revenue"] = (
        features["avg_monthly_revenue_calc"].fillna(features["monthly_revenue"]).round(2)
    )
...
def build_cohort_table(subscriptions: pd.DataFrame, snapshot: pd.Timestamp) -> pd.DataFrame:
    ...
    cohort_summary = subs.groupby("cohort_month", as_index=False).agg(
    ...
    if ended.empty:
        ...
        ended["observation_month"] = (
            ended["subscription_end_date"].dt.to_period("M").dt.to_timestamp()
        )
        ended["observation_month"] = ended[["observation_month", "cohort_month"]].max(axis=1)
        losses = ended.groupby(["cohort_month", "observation_month"], as_index=False).agg(
        ...
        return pd.Series(
            {
                ...
                "current_mrr_at_risk": round(
                    float(group.loc[group["at_risk_flag"] == 1, "current_mrr"].sum()), 2
                ),
                "churned_monthly_value_proxy": round(
                    float(group.loc[group["churn_flag"] == 1, "avg_monthly_revenue"].sum()), 2
                ),
...
def main() -> None:
    ...
    print(
        f"Features built (snapshot={snapshot.date()}). "

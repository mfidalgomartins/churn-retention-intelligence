...
class TestMonthlyDimensionalTrendEdgeCases(unittest.TestCase):
    def test_same_month_start_and_churn_can_have_zero_active_start(self) -> None:
        features = _features([
        ...
        self.assertEqual(row["segment"], "Enterprise")
        self.assertEqual(row["active_customers_start"], 0)
        self.assertEqual(row["churned_customers"], 1)
        self.assertEqual(row["churned_mrr"], 100.0)
...
class TestBehavioralRelationshipsEdgeCases(unittest.TestCase):
    def test_all_customers_in_signal_group_has_nan_out_group_metrics(self) -> None:
        ...
        failed_payment = relationships[relationships["relationship"] == "failed_payment_flag"].iloc[0]
...
class TestInterventionPrioritiesEdgeCases(unittest.TestCase):
    def test_zero_candidate_play_keeps_numeric_zeroes_and_na_focus(self) -> None:
        features = _features([
            ...
            {"customer_id": "B", "payment_failure_flag": 0, "churn_flag": 0, "current_mrr": 500.0},
        ])
